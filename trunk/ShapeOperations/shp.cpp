...
bool DeleteFile(char* nfl)
{
	wxRemoveFile(wxString::Format("%s",nfl));
	return true;
}

bool CopyFile(char* nfl, char* ofl, bool flag)
{
	wxCopyFile(wxString::Format("%s",nfl), wxString::Format("%s",ofl), flag);
	return true;
}
...
int PolygonPartition::sweep(PolygonPartition & guest, const int criteria)  
{
	...
	for (cell= 0; cell < pX.Cells(); ++cell) {
		for (host= pX.first(cell); host != GeoDaConst::EMPTY; host= pX.tail(host))
			pY.include(host);
		for (dot= guest.pX.first(cell); dot != GeoDaConst::EMPTY; dot= guest.pX.tail(dot)) {
			pt= guest.Points[dot];
			cly= pY.inTheRange(pt.y - yStart);
			if (cly != -1) {
				for (host= pY.first(cly); host != GeoDaConst::EMPTY; host= pY.tail(host)) {
					if (pt == Points[host]) {
