...
class Syslog : public ofstream  
{
public :
    Syslog(bool s = false) : screen(s) {};
    
    Syslog & operator << (const int &v)  {
        ofstream *p = this;
        (*p) << v;
        if (screen) cout << ' ' << v;
        return *this;
    }
    Syslog & operator << (const long unsigned int &v)  {
        ofstream *p = this;
        (*p) << v;
        if (screen) cout << ' ' << v;
        return *this;
    }
    Syslog & operator << (const double &v)  {
        ofstream *p = this;
        (*p) << v;
        if (screen) cout << ' ' << v;
        return *this;
    }
    Syslog & operator << (const char c[])  {
        ofstream *p = this;
        (*p) << c;
        if (screen) cout << ' ' << c;
        return *this;
    ...
    Syslog & width(const int w)  {
        ofstream *p = this;
        p->width(w);
        if (screen) cout.width(w);
        return *this;
    }
    void outline(const char * name, const double &coeff, const double &err)  {
            (*this).precision(9);			  if (screen) cout.precision(9);
            *this << "    ";			
            (*this).width(8);			 
            *this << name;		
            (*this).width(12);			  
