...
const static char W_extension[][4] = {  "_+_", "mat",  "gwt",  "gal",  };
...
inline void DeviateFromMean(double *dt, int dim)
{

	double sdt = ComputeMean(dt, dim);
	for (int i=0;i<dim;i++)
		dt[i] = dt[i] - sdt;
}

inline void DeviateFromMean(DoublePtr *dt, int dim, int var)
{
	int i = 0, j = 0;
	for (j=0; j<var; j++)
	{
...
ostream & operator << (ostream &s, pairstruct<F, S> &v)  {
