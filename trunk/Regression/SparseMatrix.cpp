...
SparseMatrix::SparseMatrix(const char * filename)  
{
    ...
    if (len > 4 && filename[len-4] == '.')  {
        for (int ch = 1; ch <= 3; ++ch)
            ext[ch-1] = tolower( filename[len-4+ch] );
        ext[3] = '\x0';
    };

    if (strcmp(ext, "gal") == 0) 
			createGAL(filename);
    else if (strcmp(ext, "gwt") == 0) 
			createGWT(filename);
//    else error("unknown file type/extension");
    int totalNonZeros = 0;
    for (int cnt = 0; cnt < size; ++cnt)
        totalNonZeros += row[cnt].getSize();
//    (*logFile) << " nonzero elements: " << totalNonZeros << " average nz elements per row: " << 1.0*totalNonZeros/size << '\n';

}

void SparseMatrix::createGWT(const char * filename)  
{
    ifstream	input(filename);
    if (!input)  error(strcat("cannot open file ", filename));
    int dim;
    input >> dim;
//    (*logFile) << "  dim: " << dim << '\n';
    ...
    Map	* key = new Map [ dim ];
    Tramp * values = new Tramp [ dim ];
    
    this->init( dim );

    int pos = 0, cnt = 0, rrow, column, oldRow;
    double w;
    input >> rrow >> column >> w;
    oldRow = rrow;
    key[cnt] = Map(rrow, cnt);

    while (!(input.eof()))  {		// read one weight
        if (oldRow == rrow)  		// continue adding elements to the existing row
            values[ pos++ ] = Tramp(column, w);	// save weight
        else  {
            if (cnt >= dim) break;
            this->row[cnt].alloc(pos);
            for (int nb = 0; nb < pos; ++nb)
                this->row[cnt].setNb( nb, values[nb].first, values[nb].second );
            ++cnt;
            key[cnt] = Map(rrow, cnt);
            oldRow = rrow;
            pos = 0;
            values[ pos++ ] = Tramp(column, w);
        };
        input >> rrow >> column >> w;
    };
    this->row[cnt].alloc(pos);
    ...
    for (cnt = 0; cnt < dim; ++cnt)  
		{
        for (int nb = 0; nb < this->row[cnt].getSize(); ++nb)  {
            int id = row[cnt].getIx(nb);
            int ix = Find( key, dim, id );
            row[cnt].setIx( nb, ix );
        };
    };
...
void SparseMatrix::createGAL(const char * filename)  
{	// get the weights from GAL file
    ifstream	input(filename);
    if (!input)
		{
			wxMessageBox("Can't open file");
		  return;
		}
    int dim;
    input >> dim;
...
void SparseMatrix::print(const char * t)  const  
{
    (*logFile) << "  sparse matrix ";
    if (t)  (*logFile) << t;
    (*logFile) << "    dim: " << size << '\n';
    int limit = (20 < size) ? 20 : size;
...
void SparseMatrix::IminusRhoThisPrime(const double rho, const DenseVector &column, DenseVector &result)  const  {
    result.copy(column);
    const double m = -rho;
    for (int r = 0; r < size; ++r)
        row[r].rowPlusSRow(result, m * column.getValue(r));
}
