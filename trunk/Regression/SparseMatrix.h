...
public :
    ...
    SparseMatrix(const char * filename);			// read sparse matrix from a file
    SparseMatrix(const SparseMatrix &W, const double rho);	// create sparse matrix I-rho*W
    ...
    void print(const char * t = NULL)  const;
    ...
    void IminusRhoThisPrime(const double rho, const DenseVector &column, DenseVector &result)  const;
...
private :
    ...
    void createGAL(const char * filename);
    void createGWT(const GwtElement * my_gwt, int obs);
    void createGWT(const char * filename);
};
#endif
