...
//  (*logFile) << " at: " << val << " v: " << one << "  ds: " << ds << " ns: " << ns << '\n';
  return ns;
}
...
void QL_with_Shift(double * d, double * e, const int dim)  {
    ...
    for (int row = 0; row < dim; ++row)  {
        ...
        do  {
            ...
            if (m != row)  {
                if (++iter == LIMIT)  {
//                    (*logFile) << "QL_with-Shift; excceding number of iterations limit\n";
                };
...
//    double xx = -1.0*((nn/2.0)*(log(2.0*M_PI))+(nn/2.0)*log((ss/nn))+(ss/(2.0*(ss/nn))));

//    (*logFile) << " rho: " << rho << "  lj: " << lj << "  sse: " << sse << "  acc: " << accum << '\n';
//    (*logFile) << "  acc1: " << accum1 << '\n';
//    (*logFile) << '.';
//    (*logFile).flush();
    return accum;
...
//    (*logFile) << "(" << x0 << ',' << f0 << "),(" << x1 << ',' << f1 << "),(" << x2 << ',' << f2 << ")  rest: " << value << '\n';
    return value;  
}
...
//  
//  (*logFile) << "\n iterations: " << Counter << "  convergence error: " << (x2-x0) << '\n';
//  (*logFile) << "     SOLUTION: " << '\n';
//  (*logFile) << '\n' << smile << "coefficient of spatial association (spatial lag model): " << x1 << '\n';
//  (*logFile) << smile << "log-likelihood: " << f1 << '\n';
...
VALUE GoldenSection(const VALUE left, const VALUE middle, const VALUE right, WVector &resid, WVector &residW)  
{
  ...
  // (*logFile) << " golden section: iterations: " << Counter << "  convergence error: " << (x3-x0) << '\n';
  return  x1;
}
...
void cg(const SparseMatrix &m, const double rho, const DenseVector &rhs, DenseVector &sol)  
{
    const int LIMIT = 50;
    double n, beta, n_lag;
    ...
    n = resid.norm();				// norm residuals
                         //(*logFile) << "  rho: " << rho << "  resid norm: " << n << '\n';
    int it = 0;
//    double  ss = 0, ss2 = 0;
    while (n > 1.0e-16 && it++ < LIMIT)  {
        if (it == 1) d.copy(resid);
        else  {
            beta = n / n_lag;
...
//        double dnorm = d.norm();
//        ss += alpha * sqrt(dnorm);
//        ss2 += dnorm / d.product(p);
//(*logFile) << "it: " << it << "  alpha: " << alpha << " d_norm: " << dnorm << "  ss: " << ss << "  ss2: " << ss2 << '\n';
    };
//(*logFile) << "it: " << it << "  resid norm: " << n << '\n';
}

void cg2(const SparseMatrix &m, const double rho, const DenseVector &rhs, DenseVector &sol)  {
    const int LIMIT = 40;
    double n, beta, n_lag;

    DenseVector		resid( m.dim() ), p( m.dim() ), d( m.dim() ), p2( m.dim() );

    m.IminusRhoThis( rho, sol, p );		// p = (I-rho*M)*sol
    m.IminusRhoThisPrime(rho, p, p2);
    resid.minus( rhs, p2 );			// residuals
    n = resid.norm();				// norm residuals
//(*logFile) << "  rho: " << rho << "  resid norm: " << n << '\n';
    int it = 0;
    while (n > 1.0e-14 && it++ < LIMIT)  {
        if (it == 1) d.copy(resid);
        else  {
            beta = n / n_lag;
            d.timesPlus( resid, beta );
        ...
        m.IminusRhoThis( rho, d, p );		// p = (I-rho*W)*d
        m.IminusRhoThisPrime( rho, p, p2 );
        double alpha = n / d.product( p2 );	// alpha = n / d'p
        sol.addTimes( d, alpha );		// sol += alpha*d
        resid.addTimes(p2, -alpha );		// resid -= alpha*p
        n_lag = n;
        n = resid.norm();
    };
    // (*logFile) << "it: " << it << "  resid norm: " << n << '\n';
...
//            cg(w, rho, rhs, sol);
            ...
            lalpha = p - pr;
// (*logFile) << "lalpha: " << lalpha << '\n';
        };
    double ll  = log_likelihood(s, dim);
    double llo = log_likelihood(ssRhs, dim);
// (*logFile) << "  ss rhs-scaled: " << ssRhs << "    ss_scaled: " << s << '\n';
    double lJ = ll - llo;
// (*logFile) << "  rho: " << rho << "  sse: " << ss << " log-likelihood: " << ll;
// (*logFile) << "log-determiant: " << lJ <<  "  ll: " << llo << '\n';
// (*logFile) << "yy: " << y.norm() << "  yWWy: " << lag.norm() << "  ee: " << ssRhs << " eWy: " << rhs.product(lag) << '\n';
    return ll;
}
...
int getCSV(const char * fname, DenseVector * &V, charPtrPtr &names, const int dim)  
{
    const char QUOTE = '"';
    const int		BUFFER_SIZE = 10240;
    ifstream		input(fname);
    if (!input)  error(strcat("cannot open file ", fname));
    char * buf = new char[ BUFFER_SIZE ];

    input.get( buf, BUFFER_SIZE );
    int len = strlen(buf);
    buf[len] = QUOTE;
    buf[len+1] = '\x0';
    ...
    int cnt = 0, cp =0;
    int nmc = 0;
    while (buf[cnt] != QUOTE) ++cnt;
    while (cnt < len)  {
        if (buf[cnt] == QUOTE)  {
            ++cnt;						// skip to next symbol
            if (buf[cnt] == QUOTE) nm[cp++] = '?';		// indicate missing variable name
            while (buf[cnt] != QUOTE)  
                nm[cp++] = buf[cnt++];
            nm[cp++] = '\x0';
            cnt++;						// skip closing quote
            ++nmc;
        };
        while (buf[cnt] != QUOTE && cnt < len) ++cnt;
    } while (cnt < len);
//    (*logFile) << smile << " file " << fname << " contains " << nmc <<" variables" << '\n';
    names = new charPtr[ nmc+1 ];
    names[nmc] = NULL;
    cp = 0;
    names[0] = &nm[0];
    for (cnt = 1; cnt < nmc; ++cnt)  {
        cp += strlen(names[cnt-1]) + 1;
        names[cnt] = &nm[cp];
    };

    V = new DenseVector[nmc];
    for (cnt = 0; cnt < nmc; ++cnt)
        V[cnt].alloc(dim);

    for (int row = 0; row < dim; ++row)  {
        char ch;
        double v;
        for (int cnt = 0; cnt < nmc; ++cnt)  {
            input >> v;
            V[cnt].setAt( row, v );
            input >> ch;
...
int getData(const char * fname, DenseVector * &X, charPtrPtr &names, const int dim)  
{
    bool ft = isQuote(fname);
    if (ft)		// this is a CASV file
        return getCSV(fname, X, names, dim);
    
    ifstream	input(fname);
    char * buf = new char [ 65536 ];
    if (!input)  error(strcat("cannot open file ", fname));
    int dimv, cols, cnt = 0;	//  number of observations, number of columns
    input >> dimv >> cols;
    if (dim != dimv)  
		{
        // (*logFile) << "dimension of the spatial weights matrix (" << dim 
 //                 << ") differ from the dimension of spatial variables (" << dimv << ")\n";
        delete [] buf;
	buf = NULL;
        return 0;
     };                                                   
//    (*logFile) << smile << "data file: " << fname
//			<< "observations: " << dim << "  variables: " << cols << '\n';
    X = cols ? new DenseVector[ cols ] : NULL;
    for (cnt = 0; cnt < cols; ++cnt)
        X[cnt].alloc( dim );
    
    names = new CharPtr [ cols ];
    for (cnt= 0; cnt < cols; ++cnt)  
		{		// read all names
            input >> buf;
            int	len = strlen(buf);
            names[cnt]= new char[len+1];
...
VALUE   ECL(WVector & resid, WVector & residW, const VALUE rho, const double * d)  
{
    ...
    // (*logFile) << par << '\n';
    // (*logFile) << par << " rho: " << rho << "  lj: " << lj << "  sse: " << sse << '\n';
	// 	(*logFile) << par << " acc: " << accum << "  acc1: " << accum1 << '\n';
    // (*logFile) << '.';
    // (*logFile).flush();
							  ...
							  bool InclConstant)  
{
    ...
    VALUE addOn = -0.5 * y.count() * log(sse/y.count());
//        (*logFile) << " lambda: " << lambda << " log-likelihood: " << (lj+addOn) << "  sse: " << sse << '\n';
    // (*logFile) << '.';
    // (*logFile).flush();
    return (lj + addOn);
}  
															...
															double *LogLik)  
{
    ...
    //  
    // (*logFile) << '\n' << smile << "iterations: " << Counter << "  convergence error: " << (x2-x0) << '\n';
    // (*logFile) << smile << "         estimated coefficient of spatial association (spatial error model): " << x1 << '\n';
    // (*logFile) << smile << "         log-likelihood: " << f1 << '\n';
    beta = new double[ X[0].count() ];
    for (int cnt = 0; cnt < (int) egls.count(); ++cnt)
        beta[cnt] = egls[cnt];
...
//#ifdef WORDS_BIGENDIAN
    ...
    // (*logFile) << smile << "  --- finished computing eigenvalues" << '\n';
    ...
    if (asym) {
		...
		//LOG_MSG("Eigenvalues: ");
    	//for (int i=0; i<dim; i++) {
		//	LOG_MSG(wxString::Format("s[%d] = %f", i, s[i]));
		//}
					   ...
					   double p_bar_max_fraction)  
{
    ...
    // (*logFile) << smile << " time elapsed: " << stop-start << '\n';
    return lambdaEstimate;
}
...
// (*logFile) << "rho:  " << rho << "  rho_ols:  " << rho_ols << "  rho_I: " << rho_I << " rho_IV: " << rho_IV << '\n';
...
// (*logFile) << " diff norm: " << dr << "  diff_I-pW: " << resid.norm() << '\n';
...
// (*logFile) << "y_rho norm: "  << y_rho.norm() << "  lag_rho norm: " << lag_rho.norm() << " sse: " << sol.norm() << '\n';
    double eWy = lag.product(sol), eWy_rho = lag_rho.product(sol);
    w.IminusRhoThis( rho_ols, sol, resid );
// (*logFile) << "  eWy: " << eWy << "  eWy_rho:  " << eWy_rho << "   sse_ols: " << resid.norm() << '\n';
...
// (*logFile)<< " golden section: iterations: " << Counter << "  convergence error: " << (x3-x0) << '\n';
//(*logFile) << "  log-likelihood: " << f1 << "  prime: " << prime << '\n';
    return  x1;
    }
...
//(*logFile) << "rd_norm: " << rd.norm() << "  lrd: " << rd.product(residW) << '\n';
    double sd = -dim * residW.norm() + 2.0 * trace * rd.product(residW) - trace2 * sse;
//(*logFile) << " rho: " << rho << "  fd: " << fd << "  sd: " << sd << '\n';
    return -fd / sd;
}
...
//    (*logFile) << "  sse: " << sse << '\n';
    ...
    DoublePtr* cov = new DoublePtr [vars];
    ...
    DoublePtr* lagDX = new DoublePtr [vars];	// X(I-lamW)WX
    ...
    DoublePtr* d2LdBeta2 = new DoublePtr [vars];
    for (cnt = 0; cnt < vars; ++cnt)  {
...
//    s1.print("dL/dBeta");
//    s5.print("dBeta/dLambda");
//    wk1.print("(partial) d2beta/dlambda2");
...
//    (*logFile) << "psd: " << psd << "  sd1: " << sd1 << "  sd2: " << sd2 << "  sd3: " << sd3 << '\n';
//(*logFile) << " fd:  " << fd << "   sd:  " << sd << '\n';
    return -fd / sd;
}
