...
void DataViewerEditFieldPropertiesDlg::CreateControls()
{
	...
	field_grid->SetColLabelValue(COL_N, "field name");
...
void DataViewerEditFieldPropertiesDlg::OnApplyButton( wxCommandEvent& ev )
{
	...
	for (std::map<wxString, int>::iterator it=fn_freq.begin();
		...
		if ((*it).second > 1) {
			wxString msg;
			msg << "Field name \"" << (*it).first << "\" appears ";
	...
	if (row > -1 && col > -1) {
		...
		if (col == COL_N) {
			...
			if (!DbfFileUtils::isValidFieldName(new_str)) {
				...
				msg << "field name.  A valid field name is between one and ";
			...
			if (fn_freq[new_str] > 1) {
				wxString msg;
				msg << "\"" << new_str << "\" is a duplicate field name and ";
