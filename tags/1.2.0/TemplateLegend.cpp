...
void TemplateLegend::AddCategoryColorToMenu(wxMenu* menu, int cat_clicked)
{
	...
	wxString cat_label = template_canvas->GetCategoryLabel(c_ts, cat_clicked);
...
void TemplateLegend::OnCategoryColor(wxCommandEvent& event)
{
	...
	wxColour col = template_canvas->GetCategoryColor(c_ts, opt_menu_cat);
	wxColourData data;
	data.SetColour(col);
	...
	if (dialog.ShowModal() == wxID_OK) {
		...
		for (int ts=0; ts<template_canvas->GetCanvasTmSteps(); ts++) {
			if (num_cats == template_canvas->GetNumCategories(ts)) {
				template_canvas->SetCategoryColor(ts, opt_menu_cat,
												  retData.GetColour());
			}
		}
