						  ...
						  wxGauge* p_bar)  
{
	...
	DoublePtr* info_matrix = new DoublePtr[deps + 2];		
							...
							wxGauge* p_bar)  
{
	...
	DoublePtr* info_matrix = new DoublePtr [vars + 2];
	...
	rr->SetSigSq(sigma2);
	/*
	 (*logFile) << "information matrix: " << vars+2;
	 for ( cnt = 0; cnt < vars+2; ++cnt)  
	 {
	 for (int rc = 0; rc < vars+2; ++rc)
	 (*logFile) << "  " << info_matrix[rc][cnt];
	 (*logFile) << '\n';
	 };
	 (*logFile) << "  ---  \n";
	 */
	...
	/*
	 (*logFile) << "inverse of the information matrix\n";
	 for (cnt = 0; cnt < vars+2; ++cnt)  {
	 for (int rc = 0; rc < vars+2; ++rc)
	 (*logFile) << "  " << info_matrix[rc][cnt];
	 (*logFile) << '\n';
